...
<div class="fixed bottom-0 right-0 z-50 m-4 flex flex-col items-end gap-2" use:portal>
	{#each $toasts as { id, data } (id)}
		<div
			...
			class="rounded-lg border border-primary bg-background text-white shadow-md"
		>
			<div
			...
			>
				...
				<button
					use:melt={$close(id)}
					class="absolute right-4 top-4 grid place-items-center rounded-full text-primary square-6"
				>
					<X class="square-4" />
				</button>
			</div>
